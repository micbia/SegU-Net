...
#pred_idx = np.arange(8328, 10000)
#pred_idx = np.arange(8328, 9150)
...
path_out = path_model+'prediction_preprocess/'
...
z, depth_MHz = 8, 20
redshift = np.loadtxt('%sredshifts_z%d_%dMHz.txt' %(path_pred, z, depth_MHz))
...
for fname in ['pca4', 'pcafit', 'gpr', 'poly']:
    # Prediction loop
    for ii in tqdm(range(pred_idx.size)):
        ...
        fname = 'dT4%s_z%d_%dMHz_i%d.bin' %(fname, z, depth_MHz, i_pred)
        x_input = t2c.read_cbin('%sdata/%s' %(path_pred, fname))
        y_true = t2c.read_cbin('%sdata/xH_z%d_%dMHz_i%d.bin' %(path_pred, z, depth_MHz, i_pred))
        xHI = t2c.read_cbin('%sdata/xHI_z%d_%dMHz_i%d.bin' %(path_pred, z, depth_MHz, i_pred))
        ...
        if(PLOT_ERROR):
            ...
            t2c.save_cbin('%spredxH_from_%s' %(path_out, fname), y_pred)
            t2c.save_cbin('%serrorxH_from_%s' %(path_out, fname), y_error)
        ...
        if(PLOT_ERROR):
            np.savetxt('%sstats_i%d_from_%s.txt' %(path_out, i_pred, fname), np.array([redshift, acc, TPR, TNR, iou, mcc, mcc_err, mean_pred, mean_error, mean_true]).T, fmt='%.3f\t'+('%.3e\t'*9)[:-1], header='eff_fact=%.4f\tRmfp=%.4f\tTvir=%.4e\t\nz\tacc\t\tprec\t\tspec\t\tiou\t\tmcc\t\terr_mcc\t\tx_pred\t\terr_x_pred\t\ty_true' %(zeta, Rmfp, Tvir))
        else:
            np.savetxt('%sstats_i%d_from_%s.txt' %(path_out, i_pred, fname), np.array([redshift, acc, TPR, TNR, iou, mcc, mean_pred, mean_true]).T, fmt='%.3f\t'+('%.3e\t'*7)[:-1], header='eff_fact=%.4f\tRmfp=%.4f\tTvir=%.4e\t\nz\tacc\t\tprec\t\tspec\t\tiou\t\tmcc\t\tx_pred\t\ty_true' %(zeta, Rmfp, Tvir))
        ...
        if(PLOT_STATS):
            ...
            plt.savefig('%smcc_i%d_from_%s.png' %(path_out, i_pred, fname), bbox_inches='tight'), plt.clf()
            plt.clf()
            ...
            for iplot in range(redshift_plot.size):
                plt.text(redshift_plot[iplot]+0.03, 0.95, round(xHI_plot[iplot],1), rotation=90)
            plt.legend()
            plt.savefig('%sstats_i%d_from_%s.png' %(path_out, i_pred, fname), bbox_inches='tight'), plt.clf()
            plt.clf()
            ...
            for iplot in range(redshift_plot.size):
                plt.text(redshift_plot[iplot]+0.03, 0.95, round(xHI_plot[iplot],1), rotation=90)
            plt.legend()
            plt.savefig('%srates_i%d_from_%s.png' %(path_out, i_pred, fname), bbox_inches='tight'), plt.clf()
            plt.clf()
        ...
        if(PLOT_MEAN):
            ...
            plt.savefig('%smean_i%d_from_%s.png' %(path_out, i_pred, fname), bbox_inches='tight'), plt.clf()
            plt.clf()
        ...
        if(PLOT_VISUAL):
            ...
            plt.savefig('%svisual_i%d_from_%s.png' %(path_out, i_pred, fname), bbox_inches='tight')
            plt.clf()
        ...
        if(PLOT_ERROR):
            ...
            # SECOND LC PLOT
            ax1 = fig.add_subplot(gs[1,0])
            im = ax1.pcolormesh(redshift, dr_xy, y_error[:,i_lc,:], cmap='jet', vmin=y_error[:,i_lc,:].min(), vmax=y_error[:,i_lc,:].max())
            ax1.contour(redshift, dr_xy, y_true[:,i_lc,:])
            ...
            plt.savefig('%serror_i%d_from_%s.png' %(path_out, i_pred, fname), bbox_inches='tight')
            plt.clf()
