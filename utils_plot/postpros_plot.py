...
loss, val_loss = np.loadtxt('loss_ep-%d.txt' %epoch)[i_tf], np.loadtxt('val_loss_ep-%d.txt' %epoch) 
lr = np.loadtxt('lr_ep-%d.txt' %epoch)[i_tf]
...
for i_nm, (nm, vnm) in enumerate(zip(name_metric, name_val_metric)): 
    if not('loss' in nm):
        metric, val_metric = np.loadtxt(nm)[i_tf], np.loadtxt(vnm)[i_tf]
        lb = nm[:nm.rfind('_')]
        ax2.plot(val_metric, color='tab:'+colours[i_nm-1], ls='--')
        ax2.plot(metric, color='dark'+colours[i_nm-1], label=lb)
