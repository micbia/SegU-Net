...
for ii in tqdm(range(pred_idx.size)):
    ...
    if(PLOT_SCORE):
        if(ii % 100 == 0):
            ...
            ax_s.set_ylim(-0.02, 1.02), ax_s.set_ylabel(r'$\rm R_2$', size=20)
            ax_s.set_yticks(np.arange(0, 1.1, 0.1))
            ax_s.set_xticks(np.arange(0, 1.1, 0.2))
            ax_s.hlines(y=np.mean(acc), xmin=-0.02, xmax=1.1, ls='--', label=r'$R^2$ = %.3f' %(np.mean(acc)), alpha=0.8, color='tab:blue', zorder=3)
            plt.legend(loc=1)
