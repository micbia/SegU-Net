...
from utils_network.networks import Unet, SERENEt, SERENEt_part2
...
with strategy.scope():
    if(os.path.exists(RESUME_MODEL)):
        ...
        if(TYPE_NET == 'serene'):
            model = SERENEt(img_shape=np.append(conf.IM_SHAPE, 1), params=hyperpar, path=PATH_OUT)
            model.compile(optimizer=OPTIMIZER, loss=LOSS, loss_weights=LOSS_WEIGHTS, metrics=[METRICS, METRICS])
        elif(TYPE_NET == 'unet_reg'):
            model = SERENEt_part2(img_shape1=np.append(conf.IM_SHAPE, 1), img_shape2=np.append(conf.IM_SHAPE, 1), params=hyperpar, path=PATH_OUT)
